...
namespace GravyEngine {
    ...
    // void Graphics2D::AddText(const Vector2 &position, Font *font, bool richText, const char *text, size_t textSize, float fontSize, const Color &color, const Rectangle &clippingRect) {
    //     if(!font || textSize == 0)
    //         return;
    ...
    //     pos.y += font->calculateYOffset(currentText.c_str(), currentText.size(), fontSize);    
    //     float originX = pos.x;
    //     float originY = pos.y;
