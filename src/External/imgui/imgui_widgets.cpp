...
static bool TryParseHex(const std::string& hex, unsigned int *hexColor) 
{
    if (hex.empty() || hex.length() < 6) // Minimum length of a valid hex color code
        return false;

    std::stringstream ss;
    ss << std::hex << hex; // Interpret hex string as hexadecimal

    ss >> *hexColor; // Extract hexadecimal value

    if (ss.fail()) // Failed to parse hex color
        return false;
...
void ImGui::TextWithColor(const char* fmt, ...)
{
    va_list args;
    va_start(args, fmt);
    ...
    // Create an std::string with the appropriate size
    std::string formattedString(len + 1, '\0');

    // Format the string
    std::vsnprintf(&formattedString[0], len + 1, fmt, args);

    va_end(args);

    // Remove the null terminator at the end, as vsnprintf adds it
    formattedString.pop_back();

    std::string tempStr = formattedString;
    ...
    while (textCur < tempStr.size())
    {
        if (tempStr[textCur] == ColorMarkerStart)
        {
            ...
            {
                ImGui::TextUnformatted(&tempStr[textStart], &tempStr[textCur - textStart]);
                ImGui::SameLine(0.0f, 0.0f);
            }

            // Process color code
            size_t colorStart = textCur + 1;
            do
            {
                ++textCur;
            }
